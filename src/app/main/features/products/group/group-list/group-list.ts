...
export class GroupList extends BaseList<IGroupModel> {
  ...
  selected = signal<IGroupModel | null>(null);
  constructor() {
    super();
    this.createData();
  }
      ...
      buttons: [
        {
          name: 'COMMONS.EDIT',
          show: () => true,
          action: (item) => this.router.navigate(['group', 'form', item.id]),
  ...
  private t(key: string): string {
    return this.translate.instant(`MAIN.FEATURES.PRODUCT_MASTER.MESSAGES.${key}`, {
      entity: this.translate.instant('MAIN.FEATURES.PRODUCT_MASTER.ENTITIES.GROUP'),
    });
  }
}
