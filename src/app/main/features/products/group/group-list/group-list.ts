...
@Component({ selector: 'app-group-list', imports: [TranslatePipe, Table, Modal, ProductDeleteModal], templateUrl: './group-list.html', styleUrl: './group-list.scss' })
export class GroupList extends BaseList<IGroupModel> {
  private route = inject(ActivatedRoute); private router = inject(Router); private service = inject(GroupService);
  private toast = inject(ToastService); private translate = inject(TranslateService);
  private modalOpen = signal(false); openModal = computed(() => this.modalOpen()); selected = signal<IGroupModel | null>(null);
  constructor() { super(); this.createData(); }
  override createData(): void { this.dataModel.set(toSignal(this.route.data)()?.['data']); this.configTable = computed(() => ({ data: this.dataModel(), titles: [{ name: 'MAIN.FEATURES.PRODUCT_MASTER.FIELDS.NAME', dataField: 'name' }], buttons: [{ name: 'COMMONS.EDIT', show: () => true, action: (item) => this.router.navigate(['group', 'form', item.id]), style: 'btn btn-primary' }, { name: 'COMMONS.DELE', show: () => true, action: (item) => { this.selected.set(item); this.modalOpen.set(true); }, style: 'btn btn-danger' }] })); }
  override reloadData(): void { this.service.search().subscribe((data) => this.dataModel.set(data)); }
  override onNewRegister(): void { this.router.navigate(['group', 'form']); }
  ...
  private t(key: string): string { return this.translate.instant(`MAIN.FEATURES.PRODUCT_MASTER.MESSAGES.${key}`, { entity: this.translate.instant('MAIN.FEATURES.PRODUCT_MASTER.ENTITIES.GROUP') }); }
}
